...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        filtered_minibatch = list(filter(lambda x: len(x.buffer) > 0 or len(x.stack) > 1, minibatch))
        while len(filtered_minibatch) > 0:
            transitions = model.predict(filtered_minibatch)
            for transition, partial_parse in zip(transitions, filtered_minibatch):
                partial_parse.parse_step(transition)
            filtered_minibatch = list(filter(lambda x: len(x.buffer) > 0 or len(x.stack) > 1, filtered_minibatch))

        for finished_parse in minibatch:
            dependencies.append(finished_parse.dependencies)
