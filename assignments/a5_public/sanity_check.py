...
def question_1h_sanity_check():
    ...
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1h: initializing and forwarding in Highway network")
    print ("-"*80)
    ...
    print("Sanity Check Passed for Question 1h: Initializing and Forwarding in Highway Network!")
    print("-"*80)
...
def question_1i_sanity_check():
    ...
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1i: initializing and forwarding in CNN")
    print ("-"*80)
    ...
    max_sentence_length = max_sentence_length_words * word_size
    batch_size = 5

    x_reshaped = torch.randn((max_sentence_length_words, char_embed_size, word_size))
    cnn = CNN(char_embed_size, word_embedding_size, word_size)
    x_conv_out = cnn(x_reshaped)
    correct_shape = [max_sentence_length_words, word_embedding_size]

    assert list(x_conv_out.size()) == correct_shape, \
        "Output size should be: {}\n but is {}\n".format(correct_shape, x_conv_out.size())

    x_reshaped_2 = torch.randn((max_sentence_length_words, char_embed_size, batch_size))

    x_conv_out_2 = cnn(x_reshaped_2)
    assert list(x_conv_out_2.size()) == correct_shape, \
        "Assertion 2: output size should be: {}\n but is {}\n".format(correct_shape, x_conv_out_2.size())

    print("Shape is right!")
    print("Sanity Check Passed for Question 1i: Creating and Forwarding in CNN!")
    print("-"*80)
...
def question_1j_sanity_check(model):
	...
	sentence_length = 10
	max_word_length = 21
	input = torch.zeros(sentence_length, BATCH_SIZE, max_word_length, dtype=torch.long)
	model_embeddings = model.model_embeddings_source
	output = model_embeddings.forward(input)
...
def question_2d_sanity_check(decoder):
    ...
    print ("-"*80)
    sequence_length = 4
    inpt = torch.zeros(1, BATCH_SIZE, HIDDEN_SIZE, dtype=torch.long)
