...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)   # Tensor: (src_len*max_word_length, b)

        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)   # Tensor: (tgt_len, b)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)   # Tensor: (tgt_len*max_word_length, b)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # since enc_hiddens_proj is b X src_len X h, and dec_hidden b X h,
        # want dec_hidden b X h X 1, when doing bmm
        # so need to unsqueeze dec_hidden at dimension=2
        # output will be b X src_len X 1, need to squeeze at dimension=2
        ...
        # alpha_t is (b X src_len), enc_hiddens is (b X src_len X 2h)
        # need to squeeze alpha_t at dim=1 before bmm with enc_hiddens, then squeeze bmm result at dim=1
